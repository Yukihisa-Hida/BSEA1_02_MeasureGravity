...
これより,4つ目の値では重大な測定ミスがあったと思われる. そのため, 4つ目の値を除外し平均処理を行うと, $\bar{g}=9.8055\,\mathrm{m/s^2}$と求められる. これは過去電気通信大学にて原子干渉計を用いた高度な重力加速度計による$g=9.7969\,\mathrm{m/s^2}$に近い値である.
...
本実験における吊り線の質量$m$は, ピアノ線の密度を$7.85\,\mathrm{g/cm^3}$とすると, $m=0.2466\,\mathrm{g}$であり, これは鋼球の質量に対して極わずかであるため, 厳密には考慮するべきだが今回の実験においては考慮する必要はない.


\subsection{課題(4)}

\begin{description}
  \item[\textcircled{\scriptsize 1}] $g$
\end{description}
